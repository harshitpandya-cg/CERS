...
const HospitalSignUpFlow: React.FC<HospitalSignUpFlowProps> = ({ onBack, onComplete }) => {
  ...
  const handleSubmit = async () => {
    ...
    // 🟢 Capture Hospital Location automatically during signup
    const getLocation = (): Promise<{ lat: number; lng: number, address: string }> => {
      return new Promise((resolve) => {
        if (!navigator.geolocation) {
          resolve({ lat: 21.1702, lng: 72.8311, address: "Surat (Fallback)" });
          return;
        }
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            const coords = { lat: pos.coords.latitude, lng: pos.coords.longitude };
            // Optional: Reverse geocode here too if needed
            resolve({ ...coords, address: "Hospital Location" });
          },
          () => resolve({ lat: 21.1702, lng: 72.8311, address: "Surat (Fallback)" }),
    ...
    const hospitalLocation = await getLocation();
    ...
    const newHospital: HospitalProfile = {
      ...
      location: hospitalLocation, // 🟢 Save coordinates
  ...
  return (
    <div className="min-h-screen bg-hospital-bg text-hospital-text flex flex-col font-sans">
      <div className="p-4 md:p-6 border-b border-gray-200 bg-white flex items-center justify-between shadow-sm">
